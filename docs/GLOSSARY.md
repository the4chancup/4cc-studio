...
| **Legacy tools** | Red, Blue, 4ccEditor, Midcupping, Rigdio/RigDJ, SEN:P-AI, the converters, the Blender addons: evidence of required behavior and formats, never source to translate. | `plans/core/README.md` "Relationship to legacy tools" |
| **Phase** | A numbered unit of the development plan (1–17), each with its own verification; the worklog tracks steps within it. | `plans/core/development_plan.md` "Development Plan" |
...
| Term | Meaning | Owner |
|---|---|---|
| **Lib crate** | A crate under `crates/libs/` holding one format or one shared concern; depends only on other libs; returns findings, never messages. | `plans/core/architecture.md` "Crate structure" |
...
| **Tool crate** | A crate under `crates/tools/`, one per tool, with the fixed skeleton `lib.rs` / `settings.rs` / `cli.rs` / `messages.rs` / `view/`. Never depends on another tool crate. | `plans/core/architecture.md` "Workspace guardrails" |
| **Tool id** | The hyphenated stable identifier of a tool (`team-compiler`), used in CLI subcommands and `MessageCode`. | `plans/core/architecture.md` "Tool plugin interface" |
...
| **FPC** | Full Player Customization: the savefile + kit-config settings that hide a player's default body so the model supplies it. Toggled per player by `fpc.on` / `fpc.off` marker files. | `plans/aesthetics_export/fpc_toggle.md` "FPC toggle"; `plans/save_editor.md` |
...
| **Help chapter** | A tool's part of the help window: the Markdown topics in its crate's `help/` folder (`StudioTool::help()`), plus a Messages topic generated from its `messages.rs` catalog. | `plans/core/gui.md` "Help window" |
| **Kit folder** | One kit's `config.toml`, `colors.txt` and textures under `Kits/`, named `<slot>[ - <label>]` (`p1 - Lakers`); `Kits/all/` holds textures every kit inherits per stem. | `plans/aesthetics_export/player_folders.md` "Kits" |
| **Kit layout** | Which engine's kit UV layout a main kit texture is drawn for (`KitLayout`: `PreFox` for PES 15–17, `Fox` for 18–21); the shirt is identical, the sock and shorts islands differ. Declared by an empty `pre-fox` / `fox` marker file in the kit folder; absent = the compile target's. | `plans/aesthetics_export/player_folders.md` "Kit layout marker" |
| **Kit slot** | A kit's position (`KitSlot`): `p1`–`p9` player kits, `g1` goalkeeper; the slot part of a kit folder name and of the game's `u0{team}{slot}` texture names. | `plans/aesthetics_export/player_folders.md` "Kits" |
| **Link file** | An empty file whose name is the reference: `Crocs.boots`, `Longhair.face`, `Keeper.gloves` point a player folder at a shared folder of that category (at most one per category); `<file>.common` points at a file in `Common/` (see "Common link"). Every link file accepts a stray `.txt` suffix (`Crocs.boots.txt`), because Windows hides known extensions and Notepad adds one. | `plans/aesthetics_export/object_model.md` "Player folders" (shared models); `plans/model_format.md` "Link files" |
| **`NO_USE` marker** | Root `NO_USE` / `NO_USE.txt` file that disables an export source; it is skipped and omitted from the grid. | `plans/team_compiler/messages.md` "Message catalog" |
...
| **Player folder** | One player's model, textures and per-player `settings.toml` inside an aesthetics export. | `plans/aesthetics_export/object_model.md` "Player folders" |
| **Player slot** | A roster position (`PlayerSlot`) in `players.txt`; `DropSlot` discards exactly one normalized assignment. | `plans/team_compiler/messages.md` "Message catalog" |
...
| **`settings.toml` / `config.toml` / `materials.toml`** | User-edited TOML inside exports (player aesthetics; kit config; model materials). Carry app-injected per-field comments; written only through `toml_edit`. | `plans/aesthetics_export/README.md`; `plans/kit_config_editor.md`; `plans/model_format.md` |
...
| Term | Meaning | Owner |
|---|---|---|
| **Disposition** | The processing consequence of a message, independent of severity: `Keep`, `DropFile`, `DropSlot`, `DropFolder`, `DropExport`, `AbortRun`. | `plans/team_compiler/messages.md` "Message structure" |
| **`DoneWithErrors`** | Folder outcome when content was written despite Error-level findings (`pass_through`, or `DropFile` with the rest processed). | `plans/core/architecture.md` "Event system" |
| **Message** | A tool's structured report to the user: `MessageCode` + severity + disposition + scope + context fields. Text is rendered from the tool's catalog, never baked in. | `plans/team_compiler/messages.md` "Message catalog" |
| **Message catalog** | A tool's table of message codes with template text and remediation hint; `messages.rs` in each tool crate. One condition, one ID. | `plans/team_compiler/messages.md` "Message catalog" |
| **`pass_through`** | Setting that turns eligible content-level `DropFile`/`DropFolder` dispositions into keep-with-flag, as in Red; never affects `DropSlot`/`DropExport`/`AbortRun`. | `plans/team_compiler/messages.md` "Message catalog" |
| **Scope** | What a message is about: `Run`, `Export`, `Folder`, `File`, `RosterEntry`; drives grid-cell mapping. | `plans/team_compiler/messages.md` "Message structure" |
| **Severity** | Presentation/filtering level: `Info` < `Warning` < `Error` < `Fatal`. Never decides processing consequence. | `plans/team_compiler/messages.md` "Message structure" |

## Formats and engines

| Term | Meaning | Owner |
|---|---|---|
| **CPK** | CRI archive format PES loads; the compiler's output container. May hold CRILAYLA-compressed entries. | `plans/libs/README.md` |
| **CRILAYLA** | CRI's bitstream LZ compression used inside CPKs. | `plans/libs/README.md` "Format references" |
| **DDS / FTEX** | DDS: the texture container for pre-Fox; FTEX: Fox Engine's texture container. `dds_convert` handles BCn encoding in-process (no texconv). | `plans/libs/README.md` |
| **FMDL / FPK** | Fox Engine model format and its package container (PES 18–21). | `plans/libs/README.md`; `plans/model_conversion/README.md` |
...
| **Global model file** | The user-facing name for a `.glb` authoring-format model, in the community's family of "fox model file" (`.fmdl`) and "pre-fox model file" (`.model`). A nickname, not an expansion of "glb"; plans and code say glTF. | `plans/model_conversion/gltf.md` "What users call it" |
...
| **`EditFile`** | `pes_savefile`'s decoded, editable representation of an `EDIT00000000` save. | `plans/pes_savefile/README.md` |
...
| **`match_feed`** | In-process typed pub/sub of live match events read from the game's memory, with snapshot replay; feeds the tracker view and the Music player. | `plans/match_tracker/README.md` |
| **Match record** | `.match.json`: the tracker's saved per-match stats/events (replaces binary `.sen`). | `plans/match_tracker/README.md` |
...
| **SEN:P-AI** | Match tracker being replaced. | `plans/match_tracker/README.md` |
