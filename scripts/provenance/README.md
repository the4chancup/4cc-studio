...
- `pes_model/` — the `.model` layout census behind the section table in
  `docs/plans/libs.md` and the count note in `docs/DECISIONS.md`.
- `fmdl_bone_matrix/` — the bind-pose noise census, gap and histogram scripts
  behind the `1e-4` merge threshold in `docs/DECISIONS.md` (`libs.md` carries
  the numbers).
- `kit_uv/` — the kit UV island measurement and diff images behind the Fox to
  pre-Fox layout notes in `docs/plans/team_compiler.md` and `docs/DECISIONS.md`.
- `calib/` — a standalone Rust crate (own `[workspace]`, so it stays out of the
  root one) that measured kit color calibration for `docs/plans/libs.md`.
